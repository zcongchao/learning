// Tutorial 12: WageCalculator.java
...
public class WageCalculator extends JFrame 
{
   ...
   public void createUserInterface()
   {
      ...
      calculateJButton.addActionListener(
         ...
         {          
             // event handler called when calculateJButton clicked
      ...
      setTitle( "Wage Calculator" ); // set title bar string
      setSize( 230, 200 );           // set window size
      setVisible( true );            // display window
      
   } // end method createUserInterface
   
   // get user input and call calculatePay
   private void calculateJButtonActionPerformed( ActionEvent event )
   {
      ...
      // gross wages for week; returned from method calculatePay
   ...
   // calculate and display wages in grossWagesJTextField
   private double calculatePay( double hours, double wages )
   {
      ...
      final double HOUR_LIMIT = 40.0;  // declare constant
      ...
      {
         // wages for first 40 hours with time and a half added
         total = ( wages * HOUR_LIMIT ) + ( hours - HOUR_LIMIT ) *
            ( 1.5 * wages );
      }

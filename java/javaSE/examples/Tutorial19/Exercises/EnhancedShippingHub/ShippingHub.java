// Exercise 19.16: ShippingHub.java
...
public class ShippingHub extends JFrame
{
   ...
   // JButton to ship Parcel
   private JButton shipJButton;

   // JPanel for Parcels to ship
   private JPanel parcelShipJPanel;

   // JList and JScrollPane for Parcels to ship
   private JList parcelShipJList;
   private JScrollPane parcelShipJScrollPane;
   ...
   // ArrayList contains Parcels being sent to truck
   private ArrayList toBeShippedArrayList = new ArrayList();
   ...
   private void createUserInterface()
   {
      ...
      arrivedAtJTextField.setBounds( 89, 14, 197, 21 );
      ...
      parcelStateJComboBox.setBounds( 19, 24, 98, 21 );
      ...
      parcelStateJList.addMouseListener(

         new MouseAdapter() // anonymous inner class
        {
            // event handler called when mouse is double clicked
            public void mouseClicked( MouseEvent event )
         {
               if ( event.getClickCount() == 2 )
               {
                  parcelStateJListMouseDoubleClicked( event );
               }
      ...
      parcelStateJScrollPane.setBounds( 19, 55, 98, 77 );
      parcelStateJPanel.add( parcelStateJScrollPane );

      // set up shipJButton
      shipJButton = new JButton();
      shipJButton.setBounds( 19, 142, 98, 26 );
      shipJButton.setText( "Ship" );
      parcelStateJPanel.add( shipJButton );
      shipJButton.addActionListener(

         new ActionListener() // anonymous inner class
         {
            // event handler called when shipJButton is pressed
            public void actionPerformed( ActionEvent event )
            {
               shipJButtonActionPerformed( event );
            }
      ...
      // set up parcelShipJPanel
      parcelShipJPanel = new JPanel();
      parcelShipJPanel.setBounds( 508, 283, 136, 120 );
      parcelShipJPanel.setBorder(
         new TitledBorder( "Parcels to Ship" ) );
      parcelShipJPanel.setLayout( null );
      contentPane.add( parcelShipJPanel );

      // set up parcelShipJList
      parcelShipJList = new JList();

      // set up parcelShipJScrollPane
      parcelShipJScrollPane = new JScrollPane( parcelShipJList );
      parcelShipJScrollPane.setBounds( 19, 24, 98, 80 );
      parcelShipJPanel.add( parcelShipJScrollPane );
      ...
      setSize( 663, 440 );        // set window size
      setVisible( true );         // display window
      
   } // end method createUserInterface

   // send parcel to truck
   private void shipJButtonActionPerformed( ActionEvent event )
   {
      // display message if truck is full
      if ( toBeShippedArrayList.size() < 5 )
      {
         ...
         if ( parcelsArrayList.size() > 0 )
         {
            ...
            {
               position++; // go to next Parcel
            }
            else
            {
               position = 0; // go to begining
            }
         
            // load information of current Parcel
            loadParcel();
      ...
      else // display error message
      {
         JOptionPane.showMessageDialog( null,
            "Truck can only hold 5 parcels.", "Limit Exceeded",
            JOptionPane.WARNING_MESSAGE );

         // user can no longer add Parcels to truck
         shipJButton.setEnabled( false );
      }

   } // end method shipJButtonActionPerformed
   ...
   private void removeJButtonActionPerformed( ActionEvent event )
   {
          ...
          parcelStateJComboBox.getSelectedItem() ) )
      {
         ...
         parcelStateArrayList.remove( newParcel );

         // reset parcelStateJList data
         parcelStateJList.setListData(
            parcelStateArrayList.toArray() );

      } // end if
      
      // remove current Parcel from parcelsArrayList
      ...
      ActionEvent event )
   {
      ...
      {
         // create temporary Parcel object
         Parcel currentParcel = ( Parcel ) parcelIterator.next();

         // add Parcel ID to parcelStateArrayList
         if ( currentParcel.getState() == state )
         {
      ...
      // display parcelStateArrayList in parcelStateJList
      parcelStateJList.setListData(
         parcelStateArrayList.toArray() );
      
   } // end method parcelStateJComboBoxActionPerformed

   // display Parcel information in JOptionPane
   private void parcelStateJListMouseDoubleClicked(
      MouseEvent event )
   {
      // selected Parcel number
      int number = Integer.parseInt( String.valueOf(
         parcelStateJList.getSelectedValue() ) );

      // create Iterator
      Iterator parcelIterator = parcelsArrayList.iterator();

      // create ArrayList with ID numbers of Parcels to be displayed
      while ( parcelIterator.hasNext() )
      {
         // create temporary Parcel object
         Parcel currentParcel = ( Parcel ) parcelIterator.next();
         ...
         // display JOptionPane
         if ( currentParcel.getParcelID() == number )
         {
            JOptionPane.showMessageDialog( null,
            "Parcel " + currentParcel.getParcelID() 
            + "\nArrived at: " + currentParcel.getArrivalTime()
            + "\nName: " + currentParcel.getName()
            + "\nAddress: " + currentParcel.getAddress()
            + "\nCity: " + currentParcel.getCity()
            + "\nState: " + currentParcel.getState()
            + "\nZip Code: " + currentParcel.getZip(),
            "Parcel Information", JOptionPane.INFORMATION_MESSAGE );
            
         } // end if
         
      } // end while

   } // end method parcelStateJListMouseDoubleClicked
